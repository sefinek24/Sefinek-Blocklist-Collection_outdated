...
async function convertToAdGuardFormat(folderPath) {
	const defaultPath = folderPath || path.join(__dirname, '../blocklist/default');
	const adGuardPath = path.join(__dirname, '../blocklist/generated/adguard');
	const generatedPath = path.join(__dirname, '../blocklist/generated');
	...
	try {
		...
		await Promise.all(
			textFiles.map(async file => {
				const fileContents = await fs.readFile(path.join(defaultPath, file.name), 'utf8');
				const now = new Date();
				...
				const categoryPath = path.join(defaultPath).replace('\\default', '\\generated\\adguard');

				// Sprawdzenie, czy katalogi adGuardPath oraz categoryPath istnieją i utworzenie ich, jeśli nie.
				try {
		...
		await Promise.all(subdirectories.map(async subdirectory => {
			await convertToAdGuardFormat(path.join(defaultPath, subdirectory.name));
		}));

		console.log(`Conversion to AdGuard format in ${defaultPath} completed successfully.`);
	} catch (err) {
		console.error(`Error converting to AdGuard format in ${defaultPath}:`, err);
	}
}
