...
const convert = async (folderPath = path.join(__dirname, '../blocklist/template')) => {
	...
	await Promise.all(txtFiles.map(async file => {
		...
		// Cache
		const cacheFolder = path.join(__dirname, `../cache/dnsmasq/${path.basename(path.dirname(thisFileName)) === 'template' ? '' : path.basename(path.dirname(thisFileName))}`);
		...
		const replacedFile = fileContent
			.replaceAll(
				/127\.0\.0\.1 localhost\.localdomain|255\.255\.255\.255 broadcasthost|ff0(?:0::0 ip6\\-mcastprefix|2::(?:2 ip6\\-allrouter|(?:1 ip6\\-allnode|3 ip6\\-allhost))s)|(?:fe80::1%lo0 |::1 (?:ip6\\-)?)localhost|ff00::0 ip6\\-localnet|127\.0\.0\.1 local(?:host)?|::1 ip6\\-loopback|0\.0\.0\.0 0\.0\.0\.0/gmu,
				'',
			)
